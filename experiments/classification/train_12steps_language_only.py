...
def main():
    ...
    save_dir = f"experiments/classification/timesclip_{TIME_STEPS}steps_language_{timestamp}"
    
    os.makedirs(f"{save_dir}/checkpoints", exist_ok=True)
    os.makedirs(f"{save_dir}/results", exist_ok=True)
    
    print("="*70)
    print(f"12步(120天)纯语言模态直接分类")
    print("="*70)
    print(f"保存目录: {save_dir}")
    print(f"时间步数: {TIME_STEPS} ({TIME_STEPS*10}天)")
    print(f"模态: 纯语言 (无视觉分支)")
    ...
    config = {
        ...
        "description": "12步纯语言模态，不使用视觉分支，训练更快"
    }
    ...
    criterion = CombinedEarlyLoss(
        ...
        contrastive_weight=0.0,  # 纯语言模态，无对比学习
