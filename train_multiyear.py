...
def train_stage1_timeseries(
    ...
    # 分层学习率
    vision_params = list(model.vision_tower.parameters())
    other_params = [p for n, p in model.named_parameters() if 'vision_tower' not in n]
...
def train_stage2_yield_prediction(
    ...
    # 冻结时间序列模型
    for param in timeseries_model.parameters():
    ...
    for epoch in range(epochs):
        ...
        for x, y_true in pbar:
            ...
            # 提取时间序列特征
            with torch.no_grad():
                features = timeseries_model.encode_timeseries(x)  # [B, d_model]
            
            # 预测产量
            y_pred = yield_model(features)  # [B, 1]
        ...
        with torch.no_grad():
            for x, y_true in test_loader:
                ...
                features = timeseries_model.encode_timeseries(x)
                y_pred = yield_model(features)
